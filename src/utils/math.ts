...
export const limitDecimals = (value: any, len: number) => {
    ...
    if (typeof value === 'string') {
        if (!isNaN(Number(value))) {
            return String(value).length > len ? String(parseFloat(value)).slice(0, len).replace(reg, '$1$2.$3') : value.replace(reg, '$1$2.$3')
        } else {
 ...
 * @param {*} value 
 */
export const limitLength = (value: any, len: number) => {
    const reg = /[^\-?\d]/g;
    // console.log(value?.length, len)
    if (typeof value === 'string') {
        if (!isNaN(Number(value))) {
            return String(value).length > len ? String(parseFloat(value)).slice(0, len).replace(reg, '') : value.replace(reg, '')
        } else {
