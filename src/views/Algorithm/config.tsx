
import React from 'react'
import { Tag } from 'antd'

export const HASH = (<Tag color='geekblue'>哈希表</Tag>)
export const DOUBLE_POINTER = (<Tag color='volcano'>双指针</Tag>)
export const DP = (<Tag color='orange'>动态规划</Tag>)
export const SLIGDING_WINDOW = (<Tag color='lime'>滑动窗口</Tag>)
export const MATRIX = (<Tag color='red'>矩阵</Tag>)
export const MERGE_SORT = (<Tag color='brown'>归并排序</Tag>)
export const DICHOTOMY = (<Tag color='tomato'>二分法</Tag>)
export const SORT = (<Tag color='teal'>排序</Tag>)
export const BF = (<Tag color='dodgerblue'>暴力</Tag>)
export const MATH = (<Tag color='orangered'>数学</Tag>)
export const ARRAY = (<Tag color='magenta'>数组</Tag>)
export const GREEDY = (<Tag color='green'>贪心</Tag>)
export const COUNT = (<Tag color='cyan'>计数</Tag>)
export const STACK = (<Tag color='gold'>栈</Tag>)
export const BFS = (<Tag color='#87d068'>广度优先搜索</Tag>)
export const DFS = (<Tag color='#2db7f5'>深度优先搜索</Tag>)
export const DIVIDE_AND_CONQUER = (<Tag color='#ff9100'>分治</Tag>)
export const RECURSION = (<Tag color='#ff9100'>递归</Tag>)
export const DOUBLY_NODE = (<Tag color='#ff9100'>双链表</Tag>)

export const EASY = (<Tag color='rgb(0, 175, 155)'>easy</Tag>)
export const MEDIUM = (<Tag color='rgb(255, 184, 0)'>medium</Tag>)
export const HARD = (<Tag color='rgb(255, 45, 85)'>hard</Tag>)
