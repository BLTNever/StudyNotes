...
const SearchForm = (
            ...
            formJson.forEach((item: any, i) => {
                ...
                if (item.type === 'COMMON_INPUT') {
                    const child = <Input type="text"
                        ...
                        value={formParams[field] ? formParams[field] : null}
                    ...
                    const child = <Select
                        // showSearch
                        value={formParams[field] ? formParams[field] : (initialValue ?? undefined)}
                        placeholder={placeholder}
                    ...
                    const child = <BrandSearch placeholder={placeholder}
                        value={formParams[field] ? formParams[field] : (initialValue?.length ? initialValue : undefined)}
                        onChange={(e: string) => onChange(e, field)}
                    />
                    const NODE = renderItem(item, child)
                    formItemList.push(NODE)
                } else if (item.type === 'COMMON_GOODS_SORT') {
                    const child = <MyTreeSelect
                        api={apiList.getBackCategoryTree}
                        setCategoryId={(e) => onChange(e, field)}
                        categoryId={formParams[field] ? formParams[field] : undefined}
                    />
                    const NODE = renderItem(item, child)
                    formItemList.push(NODE)
                }
    ...
    useEffect(() => {
        const sessionSearch = Session.get('searchStorage')
        if (props?.needStorage && sessionSearch && sessionSearch !== '{}') {
            setFormParams(JSON.parse(sessionSearch))
        }
    }, [props.needStorage])
