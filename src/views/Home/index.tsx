...
const Home = () => {
  const getEnterIcon: any = (key: number, color: string) => {
    const enterMap: { [key: string | number]: ReactNode } = {
      1: <FrownTwoTone twoToneColor={color} />,
      2: <MehTwoTone twoToneColor={color} />,
      3: <SmileTwoTone twoToneColor={color} />,
    }
    const textMap: { [key: string | number]: string } = {
      1: "未开始",
      2: "进行中",
      3: "搞掂",
    }
    return <Tooltip title={textMap[key]}>{enterMap[key]}</Tooltip>
  }
  const onBack = () => {}
    ...
    return list.map((item: any) => {
      return (
        <PageHeader
          ...
          subTitle={
            <span onClick={() => goTo(item.link)}>{item.subTitle}</span>
          }
